...
SELECT DISTINCT f1.title FROM film f1
...
WHERE f2.title = 'BUCKET BROTHERHOOD' AND f1.title != 'BUCKET BROTHERHOOD' GROUP BY f1.title
HAVING count(DISTINCT c.customer_id) >= 3 ORDER BY f1.title;
